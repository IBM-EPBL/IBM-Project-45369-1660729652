...
@app.route('/contactforplasma/<emailid>', methods=('GET', 'POST'))
def contactforplasma(emailid):
     if request.method=="POST":
           ...
           ibm_db.bind_param(prep_stmt, 2, fromemail)
           ...
           message = Mail(
            ...
            to_emails=emailid,
            subject='Plasma Donor',
            html_content='<p>Hello, Your Reqest sent  was successfull. <br><br> Thank you for choosing us.</p>')
           sg = SendGridAPIClient(
            api_key=SENDGRID_API_KEY)
           try:
                response=sg.send(message)
...
def deleteuser(useremail):
    if request.method == "GET":
        ...
        flash(f'DELETED USER:{useremail}')
