...
# def myprofile(useremail):
#  if request.method == 'GET':
#     sql=("SELECT * FROM USERS WHERE email=?",(useremail,))
#     prep_stmt = ibm_db.prepare(conn, sql)
#     ibm_db.bind_param(prep_stmt, 1, email)
#     ibm_db.execute(prep_stmt)
#     useremail= ibm_db.fetch_assoc(prep_stmt)
    
#     return render_template("myprofile.html",email=useremail)
...
#          sql=("UPDATE users SET name = ?, address= ?, city = ?, pincode = ?,bloodgroup = ?, email = ? WHERE email = ?",(name,address,city,pincode,bloodgroup,email)) 
#          prep_stmt = ibm_db.prepare(conn, sql)
